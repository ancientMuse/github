...
$(document).ready(function() {
	var sc = $("#sector").val();//  session 中保存的 部门的民称 以001-008 开始
  ...
  function onMessage(event){
    ...
    if(rs.ActionType=="Response"){
  	  $("#ZL_From").empty();
	   	$("#YXBM").empty();
	   	var newoption1=$("<option>").val(1).text("选择接受指令模块");
	   	var newoption2=$("<option>").val(1).text("选择发送指令");
  	  ...
  	  $("div[class='line tableform']").empty();
	  	...
	  	document.getElementById('messages').innerHTML += '<br />'+getNowFormatDate()+" "+ rs.MSG;
    }
      ...
      $.ajax({
				...
				success : function(data,textStatus){
					...
					if(medata.Status=="201"){  
					  ...
					  $("#ZL_From").empty();
					  $("#YXBM").empty();														   
					  var zloption=$("<option>").val(medata.I).text(medata.Icode);
  ...
  function onError(event) {
	...
	$("#submit").bind("click",function(){
		...
		if(line_number>1){
			...
			console.log("显示：；：：："+qs+" "+$("#YXBM").val()+" "+$("#ZL_From").val());
			$.ajax({
				type : "POST",
		...
		if(canchange&&val!="I"){		
			...
			for ( var j = 0; j < I_sector[index][0 + 1]; j++) {														
				$.ajax({
					...
					success : function(data){
						data=eval(data);
						var option = $("<option>").val(data[0].I_MAP).text(data[0].I);
						$("#ZL_From").append(option);
					}
				});
			}														
		}
